...
#include "PDF.h"
#include "Defines.h"
#include <Public/More_Batter/GlobalMain.h>
#include <Public/More_Batter/Mesh.h>
...
HittableList objScene()
{
    ...
    auto glass = ToPtr(new Dielectric(1.5f, RayVec3(0.7f, 0.8f, 0.9f)));
    auto dragon = ToPtr(new Mesh(ROOT_PATH + string("/data/objects/bunny.obj"), glass, 0.2f));
    auto trans = ToPtr(new Translate(dragon, RayVec3(250.0f, 250.0f, 300.0f)));
    world.add(trans);

    auto dragon1 = ToPtr(new Mesh(ROOT_PATH + string("/data/objects/bunny.obj"), nullptr,0.2f));
    auto trans1 = ToPtr(new Translate(dragon1, RayVec3(250.0f, 250.0f, 300.0f)));
    pdfList.add(trans1);
