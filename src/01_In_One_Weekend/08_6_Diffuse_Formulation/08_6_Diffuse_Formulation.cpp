...
RayVec3 rayColor(CppUtil::Basic::Ptr<Ray> ray, HittableList& world, int depth)
{
    ...
    {
        RayVec3 target = random_in_hemisphere(rec.normal);
...
#pragma omp parallel for schedule(dynamic, 8)
        for (int i = 0; i < val_ImgWidth; i++)
        {
            for (int j = 0; j < val_ImgHeight; j++)
            {
                ...
                //img.SetPixel(val_ImgWidth - 1 - i, j, Image::Pixel<RayPrecision>(sqrt(newColor.r), sqrt(newColor.g), sqrt(newColor.b)));
