...
namespace Define
{
    ...
    const RayPrecision infinity = (RayPrecision)std::numeric_limits<float>::infinity();
    const RayPrecision pi = RayPrecision(3.141592653589793);

    inline std::string rootPath()
    {
        auto config = *GStorage<Ptr<Config>>::GetInstance()->GetPtr("MainConfig");
        return *config->GetStrPtr("RootPath");
    }
